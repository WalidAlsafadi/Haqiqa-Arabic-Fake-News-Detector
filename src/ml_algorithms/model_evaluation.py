"""
Final Model Evaluation Module for Palestine Fake News Detection

Evaluates the best tuned model on the held-out test set to get unbiased performance metrics.
Generates comprehensive reports, visualizations, and saves detailed results.

Features:
- Comprehensive metrics calculation (accuracy, F1, precision, recall, AUC)
- Professional visualizations (confusion matrix, ROC curve, PR curve)
...
from src.ml_algorithms.hyperparameter_tuning import load_best_tuned_model
...
def _save_deployment_model(tuned_pipeline, model_name):
    ...
    try:
        from src.config.settings import TRADITIONAL_MODELS_DIR, BEST_TRADITIONAL_MODEL_PATH, ensure_dir_exists
        
        print("Saving final model for deployment...")
        
        ensure_dir_exists(TRADITIONAL_MODELS_DIR)
        
        import pickle
        with open(BEST_TRADITIONAL_MODEL_PATH, 'wb') as f:
            pickle.dump(tuned_pipeline, f)
        
        print(f"Final pipeline saved to: {BEST_TRADITIONAL_MODEL_PATH}")
